...
# 初始化工具1的session_state
def init_tool1_state():
    required_states = {
    ...
    for row in table.rows:
        for cell in row.cells:
            for paragraph in cell.paragraphs:
                replacements += replace_in_paragraph(paragraph, old_text, new_text)
    return replacements

def process_word_template(word_file, row_data, replace_rules):
    """处理单个Word模板替换"""
    try:
        # 加载Word文档
        ...
        for para in doc.paragraphs:
            for old_text, new_text in replace_rules:
                total_replacements += replace_in_paragraph(para, old_text, new_text)
        
        # 替换表格中的内容
        for table in doc.tables:
            for old_text, new_text in replace_rules:
                total_replacements += replace_in_table(table, old_text, new_text)
        ...
        return output, f"成功替换 {total_replacements} 处内容", True
    except Exception as e:
        return None, f"替换失败: {str(e)}", False
...
def main():
    ...
    st.header("🔄 Word+Excel批量替换工具")
    st.markdown("快速实现Word模板与Excel数据的批量替换，支持表格内文字替换，保留原格式，操作简单高效。")
    st.markdown("---")
        ...
        with col2:
            ...
            if excel_file:
                try:
                    ...
                    共 {len(excel_df)} 行数据 | {len(excel_cols)} 列
                    </div>
                    """, unsafe_allow_html=True)
                except Exception as e:
